...
let rec eval e (env : (string * int) list) : int =
    ...
    | Let(bindinglit, expr) -> 
        ...
        let newEnv = aux bindinglit env
        printf "%A"  (eval expr newEnv)
        eval expr newEnv
...
let rec freevars e : string list =
    ...
    | Var x  -> [x]
    | Let(elit, ebody) ->
    //vi ændrede den her del
        let rec aux (lit) (lit2) =
            match lit with
            | [] -> lit2
            | (str, expr) :: tail -> 
                let newLit = union (freevars expr, minus (freevars ebody, [str] )) @ lit2
                aux tail newLit
        aux elit []
...
//test expression
let e11 = Let(([("x1", CstI 3); ("x2", CstI 2); ("x3", CstI 4)]), Prim("+", Var "x2", Var "x3"))
...
let rec tcomp (e : expr) (cenv : string list) : texpr =
    ...
    | Let(elit, ebody) -> 
      ...
      //TLet(tcomp erhs cenv, tcomp ebody cenv1)
        let rec aux (list) (cenv) =
            match list with
            | [] -> tcomp ebody cenv
            | (x, expr) :: tail ->
                let newCenv = x :: cenv
                TLet(tcomp expr cenv, aux tail newCenv)
